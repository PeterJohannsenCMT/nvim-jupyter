	...
	local function process_stdout()
		...
		for _, line in ipairs(lines) do
			...
			if trimmed ~= "" and trimmed:sub(1, 1) == "{" then
				...
				elseif dec_err then
					vim.notify(
						"nvim-jupyter: JSON decode failed: " .. tostring(dec_err) .. "\nline: " .. trimmed,
						vim.log.levels.WARN
					)
				end
			elseif trimmed ~= "" then
...
function M.spawn_bridge(script)
	if not script or script == "" then
		return nil, "spawn_bridge: empty script"
	end
	if not uv.fs_stat(script) then
		return nil, ("spawn_bridge: not found: %s"):format(script)
	end
	...
	if not is_executable(python_cmd) then
		...
		if requested_cmd ~= fallback and is_executable(fallback) then
			...
			vim.schedule(function()
				vim.notify(
					("nvim-jupyter: python_cmd '%s' not found; falling back to '%s'"):format(requested_cmd, fallback),
					vim.log.levels.WARN
				)
			end)
		else
