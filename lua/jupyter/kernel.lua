...
function _inline_rl:_arm()
	if self.armed or not next(self.pending) then
		return
	end
...
local function maybe_log_handles(tag, force)
	if not vim.g.jupyter_debug_handles then
		return
	end
...
local function enqueue_stream(seq, name, text)
	if not seq or not text or text == "" then
		return
	end
	stream_queue[#stream_queue + 1] = { seq = seq, name = name, text = text }
	if vim.g.jupyter_debug_handles and (#stream_queue % 1000 == 0) then
		maybe_log_handles(
			string.format("enqueue seq=%s name=%s size=%d", tostring(seq), tostring(name), #stream_queue),
			false
		)
	end
	flush_stream_queue()
...
local function ensure_bridge()
	...
	if not script then
		vim.notify(
			"Jupyter: bridge.py not found on runtimepath; set require('jupyter.config').bridge_script",
	...
	if not br then
		vim.notify(
			("Jupyter: failed to start bridge\nscript: %s\nerror: %s"):format(script, tostring(err)),
			vim.log.levels.ERROR
		)
		return false
	end
	...
	br:on_message(function(msg)
		...
		elseif t == "error" then
			...
			if bufnr and row then
				...
				if cell and cell.start_row and not cell.absolute_lineno then
					if diag_row < cell.start_row then
						diag_row = cell.start_row
					end
				...
				if line_count > 0 then
					...
					if diag_row >= line_count then
						diag_row = line_count - 1
					end
				else
					diag_row = 0
				end
		...
		elseif t == "interrupted" then
			...
			if head then
				...
				local row = cell and cell.row
				if bufnr and row then
					ui.show_inline(bufnr, row, "[interrupt requested]", { error = true })
				end
			end
			return
...
function M.stop()
	...
	flush_stream_queue()
	pcall(function()
		M.bridge:send({ type = "shutdown" })
	end)
	if M.bridge.close then
		pcall(function()
			M.bridge:close()
		end)
	end
...
local function get_head_cell()
	local head = queue[1]
	if not head or not head.seq then
		return nil
	end
