...
local function get_pager_cfg()
	...
	for k, v in pairs(pager_cfg or {}) do
		merged[k] = v
	end
	return merged
end
...
refresh_outbuf_window = function()
	...
	if out_winid and api.nvim_win_is_valid(out_winid) then
		pcall(function()
			vim.wo[out_winid].winfixbuf = false
		end)
	end
	out_winid = nil
	return nil
...
local function open_window()
	...
	pcall(function()
		vim.wo[out_winid].winfixbuf = true
	end)
...
function M.is_visible()
	if
		out_winid
		and api.nvim_win_is_valid(out_winid)
		and out_bufnr
		and api.nvim_buf_is_valid(out_bufnr)
		and api.nvim_win_get_buf(out_winid) == out_bufnr
	then
		return true
	end
	refresh_outbuf_window()
...
function M.append(seq, text, is_error)
	...
	if s == "" or is_effectively_empty(s) then
		return
	end
	ensure_started(seq)
	local lines = {}
...
-- Command(s)
pcall(vim.api.nvim_create_user_command, "JupyterClearOut", function()
	M.clear()
end, { desc = "Clear Jupyter outbuf window" })

pcall(vim.api.nvim_create_user_command, "JupyterClearBoth", function()
	M.clear_both()
end, { desc = "Clear Jupyter outbuf and inline virtual text (current buffer)" })
...
vim.api.nvim_create_autocmd("FileType", {
	...
	callback = function(ev)
		vim.keymap.set("n", "<leader>j0", function()
			M.clear_both()
		end, { buffer = ev.buf, silent = true, desc = "Jupyter: clear outbuf + inline" })
	end,
})
