...
function M.is_skip_directive_line(line)
	return type(line) == "string" and line:match("^%s*#%s*jupyter%s*:%s*skip%s*$") ~= nil
end
...
function M.populate_cell_quickfix(opts)
	...
	local title = opts.title or string.format("Jupyter cells: %s", vim.api.nvim_buf_get_name(bufnr))
