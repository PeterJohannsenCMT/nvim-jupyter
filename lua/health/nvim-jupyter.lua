...
function M.check()
	...
	local code, out, errout = run({
		py,
		"-c",
		"import sys, jupyter_client; print(sys.version.split()[0]); print(jupyter_client.__version__)",
	})
	if code == 0 then
		local lines = {}
	...
	if bridge then
		...
		warn(
			"bridge.py not found on runtimepath; set require('jupyter').setup{ bridge_script = '/abs/path/bridge.py' }"
		)
	end
	...
	if ok_baleia then
		ok("baleia.nvim present (ANSI colors in output pane)")
	else
		warn("baleia.nvim not found (output pane will be monochrome)")
	end
end
